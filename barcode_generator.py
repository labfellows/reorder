...
class BarcodeGenerator:
    ...
    def generate_barcode(barcode_value, id, item_line_id,  key):
        ...
        if item_line_id:
            image.save(f'./barcodes/{key.capitalize()}-{id}-{item_line_id}')
        else:
            image.save(f'./barcodes/{key.capitalize()}-{id}')

    for location_id in location_ids:
        location_format = 'L000000000000'
        barcode_value = location_format[:len(location_format) - len(str(location_id))] + str(location_id)
        generate_barcode(barcode_value, location_id, False, 'location')
    logger.info('Location barcodes uploaded successfully to the barcodes directory')
    ...
    for item_line_id_dict in item_line_ids:
        ...
        for item_line in list(item_line_id_dict.values())[0]:
            ...
            generate_barcode(barcode_value, item_id, item_line, 'item')
    logger.info('Item line barcodes uploaded successfully to the barcodes directory')